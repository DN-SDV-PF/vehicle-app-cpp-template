...
static const std::string DUO_API              = "duo";         // NOLINT(runtime/string)
static const auto&       DEFAULT_API          = DUO_API;
...
IVehicleDataBrokerClient::createInstance(const std::string& vdbServiceName) {
    ...
    if (apiVariant == DUO_API) {
        logger().info("Using Duo Databroker {} API", apiVariant);
        return std::make_shared<duo::BrokerClient>(vdbServiceName);
    }
